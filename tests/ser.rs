...
fn test_serialize_datetime() {
    ...
    let (header, header_size) = generate_header(DATETIME, len as u64);
...
fn test_serialize_wrapped_json() {
    ...
    let (header, header_size) = generate_header(prefix::WRAPPED_JSON, len as u64);
...
fn test_generate_header() {
    ...
    let (header, header_size) = generate_header(prefix::UINT, u8::MAX as u64 + 1);
    let expected = vec![0, 1, prefix::UINT | SIZE_PREFIX_2BYTE];
    assert_eq!(header[..header_size].to_vec(), expected);

    let (header, header_size) = generate_header(prefix::UINT, u16::MAX as u64 + 1);
    let expected = vec![0, 0, 1, 0, prefix::UINT | SIZE_PREFIX_4BYTE];
    assert_eq!(header[..header_size].to_vec(), expected);

    let (header, header_size) = generate_header(prefix::UINT, u32::MAX as u64 + 1);
